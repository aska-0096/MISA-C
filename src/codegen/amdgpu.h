                               ...
                               const int& lds_per_block)
{
    ...
    {
        printf(
            "LDS required: %d is larger than hw LDS: %d \n", lds_per_block, arch_detail.lds_size);
        return 0;
    }
...
class amdgpu_kernel_code_t
{
    ...
    int cal_granulated_workitem_vgpr_count() { return (workitem_vgpr_count - 1) / 4; }

    int cal_granulated_wavefront_sgpr_count() { return (wavefront_sgpr_count - 8) / 4; }
...
class amdgpu_kernel_arg_t(object):
    ...
    def serialize_as_metadata(self):
        ...
        return '    - {{ .name: {:<10}, .size: {}, .offset: {:>3}, .value_kind: {}, .value_type:
{}{}}}'.format( self.name, self.size, self.offset, self.value_kind, self.value_type, misc_metadata)
        ...
        if self.mc.arch_config.code_object == AMDGPU_CODEOBJECT_V2:
            ...
            with self._indent_context():
                if self.ki.kernel_code.enable_sgpr_private_segment_buffer:
                    self._emit('enable_sgpr_private_segment_buffer = {}'.format(
self.ki.kernel_code.enable_sgpr_private_segment_buffer)) if
self.ki.kernel_code.enable_sgpr_dispatch_ptr: self._emit('enable_sgpr_dispatch_ptr = {}'.format(
self.ki.kernel_code.enable_sgpr_dispatch_ptr)) if self.ki.kernel_code.enable_sgpr_queue_ptr:
                    self._emit('enable_sgpr_queue_ptr = {}'.format(
self.ki.kernel_code.enable_sgpr_queue_ptr)) self._emit('enable_sgpr_kernarg_segment_ptr =
{}'.format(           self.ki.kernel_code.enable_sgpr_kernarg_segment_ptr)) if
self.ki.kernel_code.enable_sgpr_dispatch_id: self._emit('enable_sgpr_dispatch_id'.format(
self.ki.kernel_code.enable_sgpr_dispatch_id)) # other sgpr related to be implemented
                self._emit('user_sgpr_count = {}'.format( self.ki.kernel_code.user_sgpr_count)) if
self.ki.kernel_code.enable_sgpr_workgroup_id_x: self._emit('enable_sgpr_workgroup_id_x = {}'.format(
self.ki.kernel_code.enable_sgpr_workgroup_id_x)) if self.ki.kernel_code.enable_sgpr_workgroup_id_y:
                    self._emit('enable_sgpr_workgroup_id_y = {}'.format(
self.ki.kernel_code.enable_sgpr_workgroup_id_y)) if self.ki.kernel_code.enable_sgpr_workgroup_id_z:
                    self._emit('enable_sgpr_workgroup_id_z = {}'.format(
self.ki.kernel_code.enable_sgpr_workgroup_id_z)) self._emit('enable_vgpr_workitem_id = {}'.format(
self.ki.kernel_code.enable_vgpr_workitem_id)) self._emit('is_ptr64 = {}'.format(
self.ki.kernel_code.is_ptr64)) self._emit('float_mode = {}'.format( self.ki.kernel_code.float_mode))
                self._emit('workgroup_group_segment_byte_size = {}'.format(
self.ki.kernel_code.workgroup_group_segment_byte_size)) self._emit('kernarg_segment_byte_size =
{}'.format(                 self.ki.kernel_code.kernarg_segment_byte_size))
                self._emit('wavefront_sgpr_count = {}'.format(
self.ki.kernel_code.wavefront_sgpr_count)) self._emit('workitem_vgpr_count = {}'.format(
self.ki.kernel_code.workitem_vgpr_count)) self._emit('granulated_workitem_vgpr_count = {}'.format(
self.ki.kernel_code.granulated_workitem_vgpr_count)) self._emit('granulated_wavefront_sgpr_count =
{}'.format(           self.ki.kernel_code.granulated_wavefront_sgpr_count))
            ...
            with self._indent_context():
                if self.ki.kernel_code.kernarg_segment_byte_size > 0:
                    self._emit('.amdhsa_group_segment_fixed_size {}'.format(
self.ki.kernel_code.workgroup_group_segment_byte_size)) if
self.ki.kernel_code.enable_sgpr_dispatch_ptr: self._emit('.amdhsa_user_sgpr_dispatch_ptr {}'.format(
self.ki.kernel_code.enable_sgpr_dispatch_ptr))

                self._emit('.amdhsa_user_sgpr_kernarg_segment_ptr {}'.format(
self.ki.kernel_code.enable_sgpr_kernarg_segment_ptr))

                if self.ki.kernel_code.enable_sgpr_workgroup_id_x:
                    self._emit('.amdhsa_system_sgpr_workgroup_id_x {}'.format(
self.ki.kernel_code.enable_sgpr_workgroup_id_x)) if self.ki.kernel_code.enable_sgpr_workgroup_id_y:
                    self._emit('.amdhsa_system_sgpr_workgroup_id_y {}'.format(
self.ki.kernel_code.enable_sgpr_workgroup_id_y)) if self.ki.kernel_code.enable_sgpr_workgroup_id_z:
                    self._emit('.amdhsa_system_sgpr_workgroup_id_z {}'.format(
self.ki.kernel_code.enable_sgpr_workgroup_id_z))

                self._emit('.amdhsa_system_vgpr_workitem_id {}'.format(
self.ki.kernel_code.enable_vgpr_workitem_id)) self._emit('.amdhsa_next_free_vgpr {}'.format(
self.ki.kernel_code.workitem_vgpr_count)) self._emit('.amdhsa_next_free_sgpr {}'.format(
self.ki.kernel_code.wavefront_sgpr_count - 2*3))
                ...
                if self.mc.arch_config.arch >= 1000:
                    self._emit('.amdhsa_wavefront_size32 {}'.format(                1 if
self.ki.kernel_code.wavefront_size == 32 else 0)) self._emit('.amdhsa_workgroup_processor_mode
{}'.format(        1 if not self.ki.kernel_code.cumode else 0)) self._emit('.end_amdhsa_kernel')
        else:
            assert False
...
class amdgpu_metadata_t(mc_base_t):
    ...
    def emit_one_kernel_metadata(self, ki_):
        ...
        self._emit('    .sgpr_count: {}'.format( ki_.kernel_code.wavefront_sgpr_count)) self._emit('
.vgpr_count: {}'.format(                    ki_.kernel_code.workitem_vgpr_count)) self._emit('
.kernarg_segment_align: {}'.format(         8))     # default set to 8 self._emit('
.kernarg_segment_size: {}'.format(          ki_.kernel_code.kernarg_segment_byte_size)) self._emit('
.group_segment_fixed_size: {}'.format(      ki_.kernel_code.workgroup_group_segment_byte_size))
        ...
        self._emit('    .reqd_workgroup_size : [{}]'.format(        '{}, 1, 1'.format(
ki_.kernel_block_size) \
                                                                                if
type(ki_.kernel_block_size) is int else \
                                                                    '{}, {},
{}'.format(ki_.kernel_block_size[0], ki_.kernel_block_size[1],ki_.kernel_block_size[2])))
        self._emit('    .max_flat_workgroup_size: {}'.format(       ki_.kernel_block_size if
type(ki_.kernel_block_size) is int else \
