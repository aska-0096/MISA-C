...
public:
        ...
        : gxco::generator_t(ti_, emitter_), tunable(tunable_)
    {
        ...
        {
            /*
            assert( (tunable.lanegroup_tile_m * tunable.lanegroup_repeat_m) %
            coalescing_store_groups == 0 ); ctrl_dotx_mapping_t ctrl_dotx_mapping =
            get_ctrl_dotx_mapping_from_lanegroup_tile( tunable.gemm_m_per_block,
            tunable.gemm_n_per_block, tunable.lanegroup_tile_m, tunable.lanegroup_tile_n,
                                                    tunable.lanegroup_wave_m,
            tunable.lanegroup_wave_n, tunable.block_size / (tunable.lanegroup_wave_m *
            tunable.lanegroup_wave_n * LANEGROUP_SIZE), tunable.lanegroup_repeat_m,
            tunable.lanegroup_repeat_n, tunable.precision, get_dotx_fma_instruction(ti.arch,
            tunable.precision));
            ...
            ctrl_coalescing_store.vector_store_m =
            coalescing_store_dotx_get_optimal_vector_m(ctrl_dotx_mapping, tunable.vector_c,
            coalescing_store_groups); ctrl_coalescing_store.vector_fold_m = tunable.vector_c;
...
private:
    uint32_t get_num_vgpr_global_load_a()
    {
        ...
        uint32_t pack_factor =
            tl.ta_k_vec_c != 1 ? uint32_t(4 / amdgpu_precision_data_byte(tunable.precision)) : 1;
    ...
    uint32_t get_num_vgpr_global_load_b()
    {
        thread_lengths tl = get_thread_lengths();
        uint32_t pack_factor =
            tl.tb_nb_vec_c != 1 ? uint32_t(4 / amdgpu_precision_data_byte(tunable.precision)) : 1;
    ...
    thread_copy_index get_thread_copy_index()
    {
        ...
        std::vector<int> in_thread_copy_index =
            _find_non_1_index_in_list(tc_dims.in_thread_copy_dims);
        std::vector<int> wei_thread_copy_index =
            _find_non_1_index_in_list(tc_dims.wei_thread_copy_dims);
    ...
    //     ctrl_wei_gld.vector_d1 = ctrl_in_gld.vector_d1 = std::gcd(tunable.vector_c, 4 * int(4 /
    //     data_byte)); // max: dwordx4 access, calculate how many PIXELs accessed at once;
    ...
    void def_alias_vgpr(){
        ...
        uint32_t num_vgpr_acc_a              = (!tunable.tensor_a_pass_through) ?
    share_load_packed_vgpr * tunable.num_vgpr_accumulate_a : 0; uint32_t num_vgpr_acc_b =
    (!tunable.tensor_b_pass_through) ? share_load_packed_vgpr * tunable.num_vgpr_accumulate_b : 0;

        //printf("share_load_packed_vgpr:%d, tunable.num_vgpr_accumulate_b:%d, num_vgpr_acc_b:%d",
    share_load_packed_vgpr, tunable.num_vgpr_accumulate_b, num_vgpr_acc_b); uint32_t vgpr_cnt = 0;
        if (is_vgpr_acc_c){
            ...
            uint32_t v_c_resuable_num        = num_vgpr_acc_a + num_vgpr_acc_b +
                                               ...
                                               3 * nb_per_thread + 6;      // from v_sst_a_os to
    v_co_sst
            ...
            int v_c_needed              = (v_c_coalescing_num - v_c_resuable_num) > 0 ?
    (v_c_coalescing_num - v_c_resuable_num) : 0;

            v_c_needed              = v_c_needed > 0 ? v_c_needed : 0;  // let at least 0
            GXCO_VAR(v_c            , vgpr_cnt += v_c_needed); //f"coalescing:{v_c_coalescing_num},
    needed:{v_c_needed}, resuable:{v_c_resuable_num}"
        }
        if (!tunable.tensor_a_pass_through)
        ...
        if (tunable.fma_type == IGEMM_GTC_TUNABLE_FMA_TYPE_XDLOPS){
            if (ti.arch == gxco::GFX90A){
                ...
                total_vgpr          = total_vgpr > tunable.num_agpr_accumulate_c ? total_vgpr :
    tunable.num_agpr_accumulate_c;
        }
    ...
    */
    void create_kernel_instance()
    {
        ...
        uint32_t num_vgpr_acc_a = (!tunable.tensor_a_pass_through)
                                      ? share_load_packed_vgpr * tunable.num_vgpr_accumulate_a
                                      : 0;
        uint32_t num_vgpr_acc_b = (!tunable.tensor_b_pass_through)
                                      ? share_load_packed_vgpr * tunable.num_vgpr_accumulate_b
                                      : 0;

        // printf("share_load_packed_vgpr:%d, tunable.num_vgpr_accumulate_b:%d, num_vgpr_acc_b:%d",
        // share_load_packed_vgpr, tunable.num_vgpr_accumulate_b, num_vgpr_acc_b);
        uint32_t vgpr_cnt = 0;
        if(is_vgpr_acc_c)
        {
            GXCO_VAR(v_c, vgpr_cnt += (tunable.num_vgpr_accumulate_c + 1));
        ...
        {
            uint32_t v_c_resuable_num = num_vgpr_acc_a + num_vgpr_acc_b + num_vgpr_global_load_a +
                                        num_vgpr_global_load_b + 3 * nb_per_thread +
                                        6; // from v_sst_a_os to v_co_sst
            // v_c_coalescing_num      = tunable.num_agpr_accumulate_c / coalescing_store_groups
            uint32_t v_c_coalescing_num = coalescing_store.get_vgpr_usage();
            int v_c_needed              = (v_c_coalescing_num - v_c_resuable_num) > 0
                                 ? (v_c_coalescing_num - v_c_resuable_num)
                                 : 0;

            v_c_needed = v_c_needed > 0 ? v_c_needed : 0; // let at least 0
            GXCO_VAR(v_c,
                     vgpr_cnt += v_c_needed); // f"coalescing:{v_c_coalescing_num},
                                              // needed:{v_c_needed}, resuable:{v_c_resuable_num}"
        }
        if(!tunable.tensor_a_pass_through)
        {
         ...
         *     @subsection prepare parameters for input data loading later, e.g. strides etc. for
         * thread mapping
         ...
         *     @subsection prepare parameters for output data storing later e.g. strides etc. for
         * thread mapping
