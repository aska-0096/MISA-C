...
*/
    ctrl_dotx_mapping_t(uint32_t macro_tile_m_,       uint32_t macro_tile_n_, 
                        uint32_t lanegroup_tile_m_,   uint32_t lanegroup_tile_n_, 
                        uint32_t lanegroup_thrd_m_,   uint32_t lanegroup_thrd_n_,
                    ...
                    : macro_tile_m(macro_tile_m_), macro_tile_n(macro_tile_m_),
                      ...
                      lanegroup_wave_m(lanegroup_wave_m_), lanegroup_wave_n(lanegroup_wave_n_),
                      waves(waves_),
                      lanegroup_repeat_m(lanegroup_repeat_m_), lanegroup_repeat_n(lanegroup_repeat_n_),
                      inst_dotx(inst_dotx_)
    {
        assert((lanegroup_tile_m % LANEGROUP_SIZE == 0) && (lanegroup_tile_n % LANEGROUP_SIZE == 0));
    ...
    uint32_t wave_size(){
        return lanegroup_wave_m * lanegroup_wave_n * LANEGROUP_SIZE;
    }
    uint32_t block_size(){
        return waves * wave_size();
    }
    ...
    uint32_t lanegroup_m_per_cluster(){
        return lanegroup_tile_m/lanegroup_thrd_m;
    }

    uint32_t lanegroup_n_per_cluster(){
    ...
    void macro_tile_validate(){
        assert(macro_tile_m == lanegroup_m_per_thread() * lanegroup_m_per_cluster() * lanegroup_m_per_wave() * waves_per_m() * lanegroup_repeat_m);
        assert(macro_tile_n == lanegroup_n_per_thread() * lanegroup_n_per_cluster() * lanegroup_n_per_wave() * waves_per_n() * lanegroup_repeat_n);
        assert(macro_tile_m * macro_tile_n == block_size() * lanegroup_m_per_thread() * lanegroup_repeat_m * lanegroup_n_per_thread() * lanegroup_repeat_n);
        assert(macro_tile_m == thread_m() * lanegroup_size_m() * lanegroup_m_per_wave() * waves_per_m() * lanegroup_repeat_m);
        assert(macro_tile_n == thread_n() * lanegroup_size_n() * lanegroup_n_per_wave() * waves_per_n() * lanegroup_repeat_n);
    }

    void wave_tile_validate(){
        assert(lanegroup_m_per_cluster() * lanegroup_n_per_cluster() == LANEGROUP_SIZE);
        uint32_t wave_tile_m = lanegroup_m_per_thread() * lanegroup_m_per_cluster() * lanegroup_m_per_wave();
        uint32_t wave_tile_n = lanegroup_n_per_thread() * lanegroup_n_per_cluster() * lanegroup_n_per_wave();
